		...
		<?if($bNoSections):?>
			...
			<?foreach($arResult['SECTIONS'] as $arSection):?>		
				...
				<?if($bEditMode):?><div id="<?=$this->GetEditAreaId($arSection['ID'])?>"><?endif;?>
					...
					<div class="row row-in row-equal peoples">										
						<?foreach($arSection["ITEMS"] as $arItem):?>
							<?
								if (file_exists($_SERVER["DOCUMENT_ROOT"]."/".$this->GetFolder()."/team_list.php")){
									include($_SERVER["DOCUMENT_ROOT"]."/".$this->GetFolder()."/team_list.php");	
								}
							?>												
						<?endforeach;?>					
					</div>
